...
impl<'a> TypeChecker<'a> {
  ...
  fn top_lvl_pass(&mut self) {
    // let top_lvl = self.ast.get_stmt(self.ast.top_lvl_id())
  }
}
...
impl<'a> ast::Visitor<FrontendErrAlias> for TypeChecker<'a> {
  fn visit_expr(&mut self, id: ExprId) -> Result<TypeId, FrontendErrAlias> {
    // rn we are cloning, is this the right choice?
  ...
  fn visit_stmt(&mut self, id: StmtId) -> Result<(), FrontendErrAlias> {
    // TODO: rn we clone, is this the right
    match self.ast.get_stmt(id).clone() {
