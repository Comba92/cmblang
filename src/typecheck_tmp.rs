...
impl Typechecker {
      ...
      Expr::Call { callee, args } => {
        ...
        if let Type::FuncGeneric { params, ret } = callee_ty {
          ...
          for (param_ty_id, arg) in zip(params, args) {
            ...
            let param_ty = self.types.resolve_type_generics(param_ty_id, arg_ty_id, &mut generics_map);
              // .map_err(|e| self.err(ast, e, *span))?;

            if !self.types.ty_eq(param_ty, arg_ty_id) {
              return Err(self.err(ast, "call arguments of different type", *span))
            }

            // let param_ty = self.types.get(param_ty_id);

            // if let Type::Generic { order, .. } = param_ty {
            //   if let Some(gen_ty_id) = generics_map.get(order) {
            ...
            //     generics_map.insert(*order, arg_ty_id);
            //   }
            // } else {
          ...
          // check ret type
          // if let Type::Generic { order, .. } = self.types.get(ret) {
          //   if let Some(gen_ty_id) = generics_map.get(order) {
          ...
          // check ret type
          let ret_id = self.types.resolve_type_generics(ret, ty_id::UNTYPED, &mut generics_map);
            // .map_err(|e| self.err(ast, e, *span))?;

          if ret_id == ty_id::UNTYPED {
            return Err(self.err(ast, "can't infer generic return type", *span))
          } 
  ...
  fn check_decl(&mut self, ast: &Ast, decl: &parser::Decl, span: Span) -> Result<(), FrontendErrAlias> {
    ...
    let res = match (decl_ty, rty) {
      ...
      // (Type::Func { .. }, Type::FuncGeneric { .. }) => todo!(), 
  ...
  fn check_toplvl(&mut self, ast: &mut Ast) {
    ...
    // TODO: this step can probably be removed
    for (stmt, span) in &ast.toplvl {
      match stmt {
        ...
        StmtTopLvl::StructDecl { name, fields } => {
          ...
          let (_, present) = self.types.add_userdef(*name, Type::Struct { name: *name, fields: Vec::from_iter(fields.clone()) });
      ...
      match stmt {
        ...
        StmtTopLvl::FnDecl { name, generics, params, ret, .. } => {          
          let param_types = if generics.is_empty() {
            ...
            // we have generics
            // iter params and map each generic to its ordering id
            params.iter()
            .map(|(_, ty_id)| {
              ...
              if let Type::UserDef(ty_name) = ty {
                // lookup generics
                if let Some(idx) = generics.iter().find_map(|g| (g.0 == *ty_name).then_some(g.1)) {
                  *ty = Type::Generic { func_name: *name, order: idx }
                }
              }
          ...
          let ty = if generics.is_empty() {
            ...
            if let Type::UserDef(ty_name) = ret_ty {
              ...
              if let Some(idx) = generics.iter().find_map(|g| (g.0 == *ty_name).then_some(g.1)) {
                ...
                *ret_ty = Type::Generic { func_name: *name, order: idx }
              }
            }
    ...
    for (stmt, span) in &ast.toplvl {
      match stmt {
        ...
        StmtTopLvl::FnDecl { params, block, .. } => { 
          self.push_scope();
          for param in params {
            if self.add_var(param.0, param.1) {
              self.err(ast, "parameter with same name already declared", *span);
              continue;
            }
          }
